...
function normDecision(r: any): string {
  const raw =
    r?.decision ??
    r?.action ??
    r?.policy_action ??
...
function matches(decision: string, expect: "ALLOW" | "BLOCK") {
...
async function main() {
  ...
  for (const t of CASES) {
    const r: any = await scanPrompt(t.prompt);
    const d = normDecision(r);

    if (!matches(d, t.expect)) {
      failed++;
  ...
  assert.strictEqual(failed, 0, `Smoke policy failed:  cases -> `);
  console.log("\n🎉 SMOKE POLICY OK");
}
